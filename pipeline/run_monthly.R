...
# v3.0.0 Vision-first → text-only LLM → (optional) image LLM fallback + robust validate
...
cli::cli_h1(">> run_monthly v3.0.0 (Vision-first → text-only LLM → optional image LLM fallback + robust validate)")
...
# Default ON (can disable with GV_PREPROCESS=off). Makes image smaller/cleaner for APIs.
...
# Run text-only extraction for ALL non-empty OCR texts (no threshold gating)
...
if (use_img_fallback) {
  # only for posters that still yielded no rows from text extraction
