...
# v2.5.1 Vision (with optional preprocess) + strict month/year + per-image OpenAI fallback + debug
...
cli::cli_h1(">> run_monthly v2.5.1 (Vision+preprocess + strict month/year + per-image OpenAI fallback + debug)")
...
if (!is.null(debug_dir)) {
  ...
  envdump <- c(
    ...
    sprintf("VISION_MIN_CHARS=%s", Sys.getenv("VISION_MIN_CHARS", "60"))
  )
...
parse_filename <- function(path) {
  ...
  tibble(source_image=path, venue_guess=m[1,2], year=as.integer(m[1,3]), month_name_es=m[1,4], file_num=as.integer(m[1,5]))
}
...
pdf_to_png_once <- function(pdf_path, out_dir_) {
  base <- tools::file_path_sans_ext(basename(pdf_path)); out <- file.path(out_dir_, paste0(base, "_page1"))
  cmd <- sprintf("pdftoppm -png -f 1 -l 1 -singlefile %s %s", shQuote(pdf_path), shQuote(out))
  system(cmd, ignore.stdout = TRUE, ignore.stderr = TRUE)
  png_path <- paste0(out, ".png"); if (file.exists(png_path)) png_path else pdf_path
}
eff_dir <- if (is.null(debug_dir)) tempdir() else file.path(debug_dir, "pdf_firstpage"); dir.create(eff_dir, showWarnings=FALSE, recursive=TRUE)
...
# ---- Optional Preprocess (ImageMagick CLI) ----
# auto-orient → grayscale → deskew 40% → resize width 1600px → unsharp → strip
...
preprocess_on <- tolower(Sys.getenv("GV_PREPROCESS", "on")) != "off"
pre_dir <- if (is.null(debug_dir)) tempdir() else file.path(debug_dir, "preproc"); dir.create(pre_dir, showWarnings=FALSE, recursive=TRUE)
eff_df <- eff_df %>% mutate(preproc_path = if (preprocess_on) purrr::map_chr(effective_path, ~ preproc_image_cli(.x, pre_dir, target_width = 1600L)) else effective_path)

# Debug: sizes
if (!is.null(debug_dir)) {
  sizes <- eff_df %>% mutate(size_effective = file.info(effective_path)$size,
...
provider <- dplyr::case_when(provider_env %in% c("vision","openai","auto") ~ provider_env, TRUE ~ "auto")
...
run_openai_image_pipeline <- function(df) {
  ...
  cli::cli_alert_info("[extract_openai.R] strict-month image -> date / band / time (preprocessed={preprocess_on})")
  extract_openai(image_paths=df$preproc_path, month_es=df$month_name_es, year=df$year, source_image_ids=df$source_image)
}

# ---- MAIN ----
min_chars <- suppressWarnings(as.integer(Sys.getenv("VISION_MIN_CHARS", "60"))); if (is.na(min_chars)) min_chars <- 60L

if (nrow(eff_df) == 0) {
  all_extracted <- tibble(source_image=character(), event_date=as.Date(character()), band_name=character(), event_time=character())
  ...
  if (!exists("extract_gvision") && !exists("extract_google_vision")) stop("No Vision helper found.")
  ...
  ocr_df <- eff_df %>% left_join(ocr_raw, by="preproc_path") %>% mutate(nchar_ocr = nchar(ocr_text %||% ""))

  if (!is.null(debug_dir) && nrow(ocr_df) > 0) {
    head2 <- head(ocr_df, 2)
    for (i in seq_len(nrow(head2))) writeLines(substr(head2$ocr_text[i] %||% "", 1, 300),
                                               file.path(debug_dir, paste0("ocr_preview_", i, ".txt")))
  }

  source("pipeline/extract_openai_text.R")
  cli::cli_alert_info("[extract_openai_text.R] strict-month (band+date+time only); min_chars={min_chars}")

  llm_in   <- ocr_df %>% filter(nchar_ocr >= min_chars) %>% transmute(source_image, ocr_text, month_name_es, year)
  ...
  } else extract_openai_text(llm_in)
  ...
  } else tibble(source_image=character(), event_date=as.Date(character()), band_name=character(), event_time=character())

  all_extracted <- bind_rows(from_text, from_image) %>% distinct(source_image, event_date, band_name, event_time, .keep_all = TRUE)
}

# ---- Per-image counts & outputs ----
all_extracted <- eff_df %>% select(source_image) %>% distinct() %>% left_join(all_extracted, by="source_image")
per_image_counts <- all_extracted %>% group_by(source_image) %>% summarise(events = sum(!is.na(event_date) & nzchar(coalesce(band_name, ""))), .groups="drop")
cli::cli_alert_info("Per-image extracted events:"); print(per_image_counts, n = nrow(per_image_counts))

source("pipeline/validate.R"); cli::cli_alert_info("[validate.R] minimal + per-venue manual overrides + safe-venue-cols")
validated <- validate(extracted=all_extracted, meta=usable_meta, venues_path=venues_path, images_dir=images_dir)

perf_path <- file.path(out_dir, "performances_monthly.csv"); write_csv(validated$performances, perf_path); cli::cli_alert_success(glue("Wrote: {perf_path}"))
dir.create(agg_dir, showWarnings=FALSE, recursive=TRUE)
