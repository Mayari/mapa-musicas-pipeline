...
# v3.0.0 Vision (with optional preprocess) + text-only extraction + manual overrides + outputs
...
cli::cli_h1(">> run_monthly v3.0.0 (Vision+preprocess + text-only extract + overrides + debug)")
...
if (!is.null(debug_dir)) {
  ...
  envdump <- c(
    sprintf("OCR_PROVIDER=%s", Sys.getenv("OCR_PROVIDER", "auto")),
    ...
    sprintf("LLM_TEXT_PROVIDER=%s", Sys.getenv("LLM_TEXT_PROVIDER", "openai")),
    ...
    sprintf("VISION_MIN_CHARS=%s", Sys.getenv("VISION_MIN_CHARS", "20")),
    sprintf("ALLOW_IMAGE_FALLBACK=%s", Sys.getenv("ALLOW_IMAGE_FALLBACK", "off"))
  )
...
# ---- PDF → PNG first page (for previews) ----
...
preproc_on <- tolower(Sys.getenv("GV_PREPROCESS", "on")) != "off"
...
eff_df <- eff_df %>% mutate(preproc_path = if (preproc_on) purrr::map_chr(effective_path, ~ preproc_image_cli(.x, pre_dir, target_width = 1600L)) else effective_path)
...
# ---- Provider selection ----
provider_env <- tolower(Sys.getenv("OCR_PROVIDER", "auto"))
...
run_openai_image_pipeline <- function(df) {
  ...
  cli::cli_alert_info("[extract_openai.R] image (fallback) -> date / band / time (preprocessed={preproc_on})")
  extract_openai(image_paths=df$preproc_path, month_es=df$month_name_es, year=df$year, source_image_ids=df$source_image)
}

# ---- OCR + extraction ----
min_chars <- suppressWarnings(as.integer(Sys.getenv("VISION_MIN_CHARS", "20"))); if (is.na(min_chars)) min_chars <- 20L
allow_image_fb <- tolower(Sys.getenv("ALLOW_IMAGE_FALLBACK", "off")) == "on"

if (nrow(eff_df) == 0) {
  all_extracted <- tibble(source_image=character(), event_date=as.Date(character()), band_name=character(), event_time=character())

} else if (provider == "openai") {
  # Direct image → OpenAI (only if you explicitly force it)
  ...
  ocr_df <- eff_df %>% left_join(ocr_raw, by = "preproc_path") %>%
    mutate(nchar_ocr = nchar(ocr_text %||% ""))

  # Debug snippet
  if (!is.null(debug_dir) && nrow(ocr_df) > 0) {
    for (i in seq_len(min(3, nrow(ocr_df)))) {
      writeLines(substr(ocr_df$ocr_text[i] %||% "", 1, 400), file.path(debug_dir, paste0("ocr_preview_", i, ".txt")))
    }
  }

  # Text-only extraction (OpenAI or regex)
  source("pipeline/extract_openai_text.R")
  cli::cli_alert_info("[extract_openai_text.R] month-locked (band+date+time); min_chars={min_chars}")

  llm_in   <- ocr_df %>% filter(nchar_ocr >= min_chars) %>%
    transmute(source_image, ocr_text, month_name_es, year)

  from_text <- if (nrow(llm_in) == 0) {
    cli::cli_alert_warning(glue("Vision OCR produced < {min_chars} chars for all images."))
  ...
  # Optional image fallback (disabled by default to save OpenAI cost)
  short_df <- ocr_df %>% filter(nchar_ocr < min_chars)
  from_image <- if (allow_image_fb && nrow(short_df) > 0) {
...
cli::cli_alert_info("[validate.R] join venues + manual overrides + safe outputs")
