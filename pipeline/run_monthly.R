...
# v2.2.1 Vision (with preprocess) + strict month/year + per-image OpenAI fallback + manual overrides + debug
...
cli::cli_h1(">> run_monthly v2.2.1 (Vision+preprocess + strict month/year + per-image OpenAI fallback)")
...
if (length(poster_paths) > 0) {
  cli::cli_alert_info("Example: {paste(head(poster_paths, 3), collapse = ' | ')}")
}
