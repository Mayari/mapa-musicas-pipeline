...
# v2.0.0 Vision/OpenAI only (no Tesseract) + minimal OpenAI text pass + manual overrides + debug
...
cli::cli_h1(">> run_monthly v2.0.0 (Vision/OpenAI only + minimal + overrides + debug)")
...
# ---- Prepare effective file list (convert 1st page of PDFs to PNG) ----
pdf_to_png_once <- function(pdf_path, out_dir) {
  ...
  # pdftoppm outputs prefix-1.png; use -singlefile to get prefix.png
...
run_vision_pipeline <- function(df) {
  # Try both helper filenames; we keep your extract_gvision.R name too
  ...
  if (!exists("extract_gvision") && !exists("extract_google_vision")) {
    stop("No Vision helper found (expected pipeline/extract_gvision.R or pipeline/extract_google_vision.R).")
  }
...
run_openai_image_pipeline <- function(df) {
  source("pipeline/extract_openai_image.R")
  cli::cli_alert_info("[extract_openai_image.R] v2025-09-02 minimal direct image → {date,band,time}")
  # Pass month/year hints derived from filename to reduce errors
