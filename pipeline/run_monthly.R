...
# v2.3.0 Vision (with preprocess) + strict month/year + per-image OpenAI fallback + manual overrides + deep debug
...
cli::cli_h1(">> run_monthly v2.3.0 (Vision+preprocess + strict month/year + per-image OpenAI fallback + debug)")

# ---- Args ----
args <- commandArgs(trailingOnly = TRUE)
get_arg <- function(flag, default = NULL) {
  i <- which(args == flag); if (length(i) == 0 || i == length(args)) return(default); args[i + 1]
}
...
# ---- Light preprocessing (ImageMagick CLI) ----
preproc_image_cli <- function(in_path, out_dir, target_width = 1800L) {
  ...
  out <- file.path(out_dir, basename(in_path))
...
eff_df <- eff_df %>% mutate(preproc_path = purrr::map_chr(effective_path, ~ preproc_image_cli(.x, pre_dir, target_width = 1800L)))

# For debug: record file sizes pre/post
if (!is.null(debug_dir)) {
  sizes <- eff_df %>%
    mutate(size_effective = file.info(effective_path)$size,
           size_preproc   = file.info(preproc_path)$size) %>%
    arrange(desc(size_preproc))
...
run_openai_image_pipeline <- function(df) {
  ...
  extract_openai(image_paths=df$effective_path, month_es=df$month_name_es, year=df$year, source_image_ids=df$source_image)
}
...
if (nrow(eff_df) == 0) {
  ...
  ocr_raw <- gv(eff_df$preproc_path, language_hints=c("es","en"), feature="DOCUMENT_TEXT_DETECTION") %>% rename(preproc_path=source_image)
  ocr_df <- eff_df %>% left_join(ocr_raw, by="preproc_path") %>% mutate(nchar_ocr = nchar(ocr_text %||% ""))

  # Debug: write the first 300 chars of OCR for first 2 posters
  ...
  # Fallback to image-only for short OCR
...
perf_path <- file.path(out_dir, "performances_monthly.csv"); write_csv(validated$performances, perf_path); cli::cli_alert_success("Wrote: {perf_path}")
dir.create(agg_dir, showWarnings = FALSE, recursive = TRUE)
