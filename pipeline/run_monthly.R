...
# v3.1.0 Vision-first → text-only LLM → optional image LLM fallback + robust validate (prints throttle)
...
cli::cli_h1(">> run_monthly v3.1.0 (Vision-first → text-only LLM → optional image LLM fallback + robust validate)")
...
eff_df <- eff_df %>% mutate(preproc_path = if (preprocess_on) purrr::map_chr(effective_path, ~ preproc_image_cli(.x, pre_dir)) else effective_path)
